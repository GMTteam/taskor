...
interface CategoryStoreState {
    ...
    updateDragAndDrop: (categoryId: string, newTaskList: TaskItemData[]) => void;
    updateCategoriesOrder: (newOrder: CategoryType[]) => void;
    updateTask: (categoryId: string, taskData: TaskItemData) => void;
...
const useCategoryStore = create<CategoryStoreState>((set) => ({
    ...
    updateDragAndDrop: (categoryId, newTaskList) =>
        set((state) => {
            const updatedCategories = state.categories.map((category) => {
                if (category.id === categoryId) {
                    return {
    ...
    updateTask: (categoryId, taskData) =>
        set((state) => {
            const updatedCategories = state.categories.map((category) => {
                if (category.id === categoryId) {
                    const updatedTasks = category.listTask.map((task) => {
    ...
    initializeStore: async () => {
        try {
            ...
            const categories: CategoryType[] = storedCategories
                ...
                : [];
            set({ categories });
            console.log('initialize category store successfully with data', categories);
        } catch (error) {
